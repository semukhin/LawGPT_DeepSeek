...
async def send_custom_request(user_query: str, thread_id: Optional[str] = None, db: Optional[Session] = None) -> str:
    ...
    if not user_query:
        raise HTTPException(
            status_code=400, 
            detail="Необходимо предоставить текстовый запрос"
        )
    
    # Проверяем тип запроса до начала поиска
    is_general = is_general_query(query=user_query)
    if is_general:
        ...
        if thread_id and db:
            try:
                ...
                logging.info(f"📝 Загружено {len(chat_history)} предыдущих сообщений")
            except Exception as e:
    ...
    # Для юридических запросов продолжаем существующую логику
    logging.info("📚 Распознан юридический запрос, выполняем поиск")      
    ...
    max_query_size = 5000  # Ограничение на размер изначального запроса
    ...
    prompt_footer = f"""
            ...
            2. Для ЮРИДИЧЕСКИХ запросов (должен быть развёрнутый ответ):
