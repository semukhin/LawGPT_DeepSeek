...
async def send_custom_request(user_query: str, thread_id: Optional[str] = None, db: Optional[Session] = None) -> str:
    ...
    system_prompt = """
                ...
                3. Ты не должен рекомендовать пользователю обращаться к kad.arbitr.ru, СПС "Гарант" или "КонсультантПлюс", "СудАкт" (sudact.ru) или "Кодекс" или kad.arbitr.ru.
                ...
                Не выдумывай не существующие номера судебных дел (например не используй - А40-123456/2020, А41-78901/2021, А40-12345/01-12-123, А60-78901/2021 
                или А40-123456/2019), а используй только те номера которые тебе поступили с промтом.
                
                6.Если нужно привести пример судебной практики, но конкретные номера дел не указаны 
                в материалах, используй обобщенные формулировки без номеров: "согласно судебной практике...", 
                "как отмечается в решениях Верховного Суда..." и т.д.
    ...
    async def search_elasticsearch():
        try:
            ...
            if es_results:
                logging.info(f"✅ Найдено {len(es_results)} релевантных чанков в ElasticSearch")
                # Отформатируем результаты
                formatted_results = "\n\n## Российское законодательство:\n\n"
                for chunk in es_results:
                    formatted_results += f"{chunk}\n\n---\n\n"
    ...
    async def search_internet():
        try:
            ...
            web_results = await run_multiple_searches(user_query, logs)
    ...
    try:
        # Добавляем историю диалога, если доступна база данных и ID треда
        additional_context = ""
        chat_history = []
        if thread_id and db:
            try:
        ...
        all_messages.append({"role": "system", "content": conversation_system_prompt})
        ...
        # ==========================================
        # Для юридических запросов в той же функции:
        # ==========================================

        # Заменить блок загрузки истории диалога:
